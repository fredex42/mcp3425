...
double convert_to_voltage(int16_t reading) {
  double factor = (double)reading/(double)0x7FFF;
  return factor * 2.048;  //built-in reference voltage, p. 1 of datasheet
}
...
int main(int argc, char *argv) {
  ...
  while(1){
    ...
    fprintf(stdout, "Got data 0x%04x (%f V) and config 0x%02x\n", r.value, convert_to_voltage(r.value), r.config);
    usleep(1000000);
  }
}
